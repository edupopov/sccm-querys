/*
    Lista de dispositivos Windows com:
    - Hostname (coluna)
    - Sistema Operacional + versão/build (coluna)
    - Autor: Eduardo Popovici - Comentários e organização do cod feito pelo Copilot

    Banco: CM_BRF
    Views: v_R_System (rs), v_GS_OPERATING_SYSTEM (os)

    Notas:
    - Windows 11 25H2 = build 26200.xxxx (base 26200)
    - Windows 11 24H2 = build 26100.xxxx (base 26100)
*/

;WITH Base AS (
    SELECT
        rs.ResourceID,
        COALESCE(NULLIF(rs.Name0,''), NULLIF(rs.Netbios_Name0,'')) AS Hostname,
        os.Caption0,
